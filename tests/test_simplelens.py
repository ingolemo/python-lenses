...
# library.
def test_lens_and():
    my_lens = SimpleLens.both() & SimpleLens.getitem(1)
    assert my_lens.set([(0, 1), (2, 3)], 4) == [(0, 4), (2, 4)]


def test_lens_decode():
    assert SimpleLens.decode().get(b'hello') == 'hello'
    assert SimpleLens.decode('utf-8').get(b'caf\xc3\xa9') == 'caf\xe9'
    assert SimpleLens.decode('ascii', 'replace').set(b'', '\xe9') == b'?'


def test_lens_getattr():
    Tup = collections.namedtuple('Tup', 'attr')
    obj = Tup(1)
    assert SimpleLens.getattr('attr').get(obj) == 1
    assert SimpleLens.getattr('attr').set(obj, 2) == Tup(2)


def test_lens_getitem():
    assert SimpleLens.getitem(0).get([1, 2, 3]) == 1
    assert SimpleLens.getitem(0).set([1, 2, 3], 4) == [4, 2, 3]


def test_lens_trivial():
    obj = object()
    assert SimpleLens.trivial().get(obj) is obj
    assert SimpleLens.trivial().set(obj, None) is None


def test_lens_both():
    assert SimpleLens.both().get(['1', '2']) == '12'
    assert SimpleLens.both().set(['1', '2'], 4) == [4, 4]
...
def test_lens_tuple_l():
    data = {'hello': 0, 'world': 1}
    get = SimpleLens.getitem
    my_lens = SimpleLens.tuple(get('hello'), get('world'))
    assert my_lens.get(data) == (0, 1)
    assert my_lens.set(data, (3, 4)) == {'hello': 3, 'world': 4}


def test_lens_traverse():
    traversal = SimpleLens.traverse()
